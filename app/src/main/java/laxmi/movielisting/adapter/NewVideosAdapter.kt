...
class NewVideosAdapter(mainActivityViewModel: MainActivityViewModel) : RecyclerView.Adapter<NewVideosAdapter.MovieViewHolder>() {
    var newVideos = ArrayList<MovieClass>()
    val viewModel = mainActivityViewModel

    /*
    Method to set the movie data initially
    */
    fun setData(newVideos: ArrayList<MovieClass>) {
    ...
    */
    fun updateData(newVideos: ArrayList<MovieClass>) {
        val lastSize = this.newVideos.size
        this.newVideos.addAll(newVideos)
        val newSize = lastSize + newVideos.size
    ...
    class MovieViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        // Splitting year from the string
    ...
    override fun onBindViewHolder(holder: MovieViewHolder, position: Int) {
        holder.bindData(newVideos.get(position))
        if (position == newVideos.size - Constants.LAZY_LOAD_LIMIT && RetrofitApi.PAGE < Constants.PAGE_MAX) {
